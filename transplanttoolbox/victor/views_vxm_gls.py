...
from virtual_crossmatch import vxm_gls
...
class GenotypeListStringApiView(generics.GenericAPIView):
    serializer_class = serializers.VICTOR_GLstringSerializer

    def post(self, request, format=None):
        """Computes VXM when donor typing is available as genotype list string. Results also give the probability of VXM for each conflicting unacceptable antigen."""
        """parameters: 
        allele:string
        """
        serializer = serializers.VICTOR_GLstringSerializer(data=request.data)    
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            #serializer.create()
            donor_gls = serializer.data.get('Donor_GL_string')
            ...
            output = virtual_crossmatch.vxm_gls(donor_gls, pop, ua_list_split)
